    ...
    return (
        <div className="sudoku">
            <h1 className="text-4xl">SUDOKU</h1>
                ...
                <label className=" font-semibold mt-2">Difficulty:</label>
                    ...
                    row.map((num, cIdx) => (
                        <div className="" key={`${rIdx}-${cIdx}`}>
                            <input key={`${rIdx}-${cIdx}`} type="text" maxLength="1" value={num || ""} disabled={originalGrid[rIdx][cIdx] !== 0} onChange={(e) => handleChange(rIdx, cIdx, e.target.value)} 
                                className={`text-center border sudoku-grid
            ...
            <div className="mt-4 space-x-2">
                ...
                <button onClick={handleCheck} disabled={checkAttempts <= 0 || gameOver} className={`px-4 py-2 rounded ${checkAttempts <= 0 || gameOver || !isRunning ? "bg-gray-500 cursor-not-allowed" : "bg-green-500"} text-white`}>
